...
- 仅将这些特定的中间文件移动到draft子目录：
  ...
  - {directory}/{base_name}_structured.md → {directory}/draft/{base_name}_structured.md
- **不要移动**这些文件 - 将它们保留在原始目录中：
  - {directory}/{base_name}.{extension}（原始输入文件 - 保留在原始目录）
  - {directory}/{base_name}_rewritten.md（保留在原始目录 - 最终输出）
...
- 严格按照顺序执行步骤 - 不允许并发处理
- 第4步（内容覆盖检查）是绝对必须的 - 在任何情况下都不能跳过或省略
- 谨慎处理文件操作以避免前次运行的干扰
- 支持.md和.txt文件格式作为输入
...
- 步骤3：读取rewritten.md，执行措辞优化，写入到："/Users/ken/docs/report_refined.md"
...
- 步骤5最终整理：
  ...
  - "/Users/ken/docs/report_rewritten.md" → "/Users/ken/docs/draft/report_rewritten.md"
  - "/Users/ken/docs/report.txt"（原始输入文件 - 保留在原始目录）
  - "/Users/ken/docs/report_refined.md"（保留在原始目录 - 最终输出）
...
- 第4步（内容覆盖检查）仍然是作为质量检查点的必须步骤

**标准输出处理原则：**
- 此流程中的斜杠命令（步骤1、步骤2、步骤4）都输出内容到标准输出
- 您必须捕获每个命令的标准输出
- 将捕获的内容写入指定的输出文件
