...
2. **语义换行优化** - 对于过长的段落（超过5-6个句子），可根据语义在适当的句子边界处插入换行符，提高可读性，但不得改变原文意义
3. 至少3级层次结构（必须包含1级章节、2级小节和3级子小节）
4. 使用指定的编号模式
...
def semantic_line_break(text):
    ...
    for i in range(0, len(sentences), 2):
        if i < len(sentences):
            ...
            # 每5-6个句子换行
            if sentence_count >= 5 and punctuation:
...
### Q: 语义换行的标准是什么？
A: 以句号、问号、感叹号为自然边界，每5-6个句子为一个段落，保持逻辑完整性。
