...
3. **标题严格保护**：绝对禁止修改任何章节标题（## 和 ###），包括但不限于：
   - 禁止修改标题文字内容
   - 禁止改变标题层级关系
   - 禁止调整标题位置顺序
   - 禁止添加或删除标题
4. **内容完整性**：在改善表达方式时确保不丢失任何实质信息
5. **专业性保持**：维持原有的专业文档风格和术语使用
6. **标题验证要求**：处理完成后必须对比验证所有标题与原文件完全一致
7. **改善平衡性**：在提升可读性的同时避免过度装饰化
8. **可视化克制原则**：严格遵循表格→嵌套列表→Mermaid图表的优先级选择，避免过度使用图表
