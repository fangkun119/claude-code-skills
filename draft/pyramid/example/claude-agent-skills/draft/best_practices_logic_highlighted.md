...
**由此可见**，好的 Skill 应该简洁、结构良好且经过真实使用测试。**因此**，本指南提供实用的创作决策，帮助您编写 Claude 能够有效发现和使用的 Skill。
...
**基于资源限制理论**，[上下文窗口](/zh-CN/docs/build-with-claude/context-windows)是一种公共资源。**导致**您的 Skill 与 Claude 需要了解的所有其他内容共享上下文窗口，包括：
...
**值得注意的是**，Skill 中的每个令牌都没有直接成本。**然而**，启动时，只有所有 Skill 的元数据（名称和描述）被预加载。Claude 仅在 Skill 变得相关时才读取 SKILL.md，并根据需要读取其他文件。**但是**，在 SKILL.md 中保持简洁仍然很重要：**因为**一旦 Claude 加载它，每个令牌都会与对话历史和其他上下文竞争。

#### 1.1.2 默认假设：Claude 的智能基础

**默认假设**：Claude 已经非常聪明

**基于这一假设**，只添加 Claude 没有的上下文。质疑每一条信息：
...
**这个对比清楚地表明**，简洁版本假设 Claude 知道什么是 PDF 以及库如何工作。

### 1.2 自由度设置：精确控制指导程度

#### 1.2.1 自由度层次与任务匹配

**根据任务特性分析**，将具体程度与任务的脆弱性和可变性相匹配。
...
**正如导航一样**，不同场景需要不同的指导策略：

* **两侧都是悬崖的狭窄桥**：只有一种安全的前进方式。**因此**，提供具体的护栏和精确的说明（低自由度）。**例如**，必须按精确顺序运行的数据库迁移。
* **没有危险的开放田野**：许多路径都能成功。**所以**，给出一般方向并相信 Claude 会找到最佳路线（高自由度）。**比如**，上下文决定最佳方法的代码审查。

### 1.3 跨模型测试策略

#### 1.3.1 多模型兼容性测试

**基于模型差异理论**，Skill 作为模型的附加功能，**因此**其有效性取决于底层模型。**关键在于**，使用您计划使用的所有模型测试您的 Skill。

**按模型的测试考虑**：

* **Claude Haiku**（快速、经济）：**首先需要评估**，Skill 是否提供了足够的指导？
* **Claude Sonnet**（平衡）：**重点考察**，Skill 是否清晰高效？
* **Claude Opus**（强大的推理）：**需要确认**，Skill 是否避免过度解释？

**这个对比清楚地说明**，对 Opus 完美有效的东西可能需要为 Haiku 提供更多细节。**因此**，如果您计划在多个模型中使用您的 Skill，请针对所有模型都能很好地工作的说明。
...
**基于用户体验研究**，使用一致的命名模式使 Skill 更容易引用和讨论。**专家建议**，对 Skill 名称使用**动名词形式**（动词 + -ing），**因为**这清楚地描述了 Skill 提供的活动或能力。

**值得注意的是**，`name` 字段必须仅使用小写字母、数字和连字符。

**以成功案例为例**，**好的命名示例（动名词形式）**：
...
`description` 字段启用 Skill 发现，**应该**包括 Skill 的功能和使用时机。
...
**基于发现机制分析**，**具体并包含关键术语**。包括 Skill 的功能和使用它的具体触发器/上下文。

**关键在于**，每个 Skill 恰好有一个描述字段。**事实上**，描述对于 Skill 选择至关重要：**因为**Claude 使用它从可能的 100+ 个可用 Skill 中选择正确的 Skill。**因此**，您的描述必须提供足够的细节，以便 Claude 知道何时选择此 Skill，而 SKILL.md 的其余部分提供实现细节。
...
**基于认知负荷理论**，SKILL.md 作为概述，指向 Claude 根据需要查看的详细材料，**正如**入职指南中的目录一样。有关渐进式披露如何工作的解释，请参阅概述中的[Skill 如何工作](/zh-CN/docs/agents-and-tools/agent-skills/overview#how-skills-work)。
...
**这种设计的优势在于**，Claude 仅在需要时加载 FORMS.md、REFERENCE.md 或 EXAMPLES.md。

#### 2.4.4 组织模式二：特定领域分组

##### 模式 2：特定领域组织

**基于信息架构原理**，对于具有多个领域的 Skill，**按领域组织内容**可以避免加载无关的上下文。**具体来说**，当用户询问销售指标时，Claude 只需要读取与销售相关的架构，而不是财务或营销数据。**因此**，这保持令牌使用低且上下文集中。
...
**这种模式的优点是**，Claude 仅在用户需要这些功能时读取 REDLINING.md 或 OOXML.md。
...
**基于信息检索理论**，当从其他引用文件引用文件时，Claude 可能会部分读取文件。**遇到嵌套引用时**，Claude 可能会使用 `head -100` 等命令预览内容，而不是读取整个文件，**导致**信息不完整。

**因此**，**保持引用距离 SKILL.md 一级**。所有参考文件应直接从 SKILL.md 链接，以确保 Claude 在需要时读取完整文件。
...
**实践证明**，对于超过 100 行的参考文件，在顶部包含目录。**这样做的优势在于**，确保 Claude 即使在部分读取时也能看到可用信息的完整范围。
...
**因此**，Claude 可以根据需要读取完整文件或跳转到特定部分。
...
**基于任务分解理论**，将复杂操作分解为清晰的顺序步骤。**对于特别复杂的工作流**，提供一个清单，Claude 可以将其复制到其响应中并在进行时检查。

#### 3.1.2 无代码技能工作流示例

**以研究综合为例**，**示例 1：研究综合工作流**（适用于没有代码的 Skill）：
...
**这个例子清楚地表明**，工作流如何应用于不需要代码的分析任务。**值得注意的是**，清单模式适用于任何复杂的多步骤流程。

#### 3.1.3 有代码技能工作流示例

**以 PDF 表单填充为例**，**示例 2：PDF 表单填充工作流**（适用于有代码的 Skill）：
...
**由此可见**，清晰的步骤防止 Claude 跳过关键验证。**这个清单的作用是**，帮助 Claude 和您跟踪多步骤工作流的进度。
...
**实践证明**，**常见模式**：运行验证器 → 修复错误 → 重复

**重要的是**，此模式大大提高输出质量。

#### 3.2.2 无代码验证循环示例

**以风格指南为例**，**示例 1：风格指南合规性**（适用于没有代码的 Skill）：
...
**这个例子清楚地展示了**，使用参考文档而不是脚本的验证循环模式。"验证器"是 STYLE\_GUIDE.md，Claude 通过读取和比较来执行检查。

#### 3.2.3 有代码验证循环示例

**以文档编辑为例**，**示例 2：文档编辑流程**（适用于有代码的 Skill）：
...
**这种设计的优势在于**，旧模式部分提供历史背景，而不会使主要内容混乱。
...
**基于认知一致性理论**，选择一个术语并在整个 Skill 中使用它：
...
**基于标准化理论**，为输出格式提供模板。将严格程度与您的需求相匹配。
...
**基于示范学习理论**，对于输出质量取决于看到示例的 Skill，提供输入/输出对，**正如**在常规提示中一样：
...
**实践证明**，示例帮助 Claude 比单独的描述更清楚地理解所需的风格和细节程度。
...
**基于精益开发原则**，**在编写大量文档之前创建评估**。**关键在于**，这确保您的 Skill 解决真实问题，而不是记录想象的问题。
...
**这种方法的优势在于**，确保您解决实际问题，而不是预期可能永远不会出现的要求。
...
**基于协作开发理论**，最有效的 Skill 开发流程涉及 Claude 本身。**具体来说**，与一个 Claude 实例（"Claude A"）合作创建将由其他实例（"Claude B"）使用的 Skill。Claude A 帮助您设计和改进说明，而 Claude B 在真实任务中测试它们。**这种模式有效的根本原因在于**，Claude 模型既理解如何编写有效的代理说明，也理解代理需要什么信息。
...
**基于持续改进理论**，当改进 Skill 时，相同的分层模式继续。您在以下之间交替：
...
**为什么此方法有效**：**基于多方协作理论**，Claude A 理解代理需求，您提供领域专业知识，Claude B 通过真实使用揭示差距，迭代改进根据观察到的行为而不是假设改进 Skill。
...
**基于用户体验研究**，当您迭代 Skill 时，注意 Claude 实际上如何在实践中使用它们。观察：

* **意外的探索路径**：Claude 是否以您没有预期的顺序读取文件？**这可能表明**，您的结构不如您认为的那样直观
* **错过的连接**：Claude 是否未能遵循对重要文件的引用？**这种情况下**，您的链接可能需要更明确或突出
* **对某些部分的过度依赖**：如果 Claude 反复读取同一文件，考虑该内容是否应该在主 SKILL.md 中
* **忽略的内容**：如果 Claude 从不访问捆绑文件，它可能是不必要的或在主说明中信号不良

**因此**，根据这些观察而不是假设进行迭代。**值得注意的是**，您的 Skill 元数据中的"name"和"description"特别关键。**因为**Claude 在决定是否响应当前任务触发 Skill 时使用这些。确保它们清楚地描述 Skill 的功能和使用时机。
...
**基于跨平台兼容性原则**，始终使用正斜杠在文件路径中，即使在 Windows 上：

* ✓ **好的**：`scripts/helper.py`、`reference/guide.md`
* ✗ **避免**：`scripts\helper.py`、`reference\guide.md`

**实践证明**，Unix 风格的路径在所有平台上都有效，而 Windows 风格的路径在 Unix 系统上会导致错误。
...
**基于责任驱动设计**，编写 Skill 脚本时，处理错误条件而不是推卸给 Claude。
...
**值得注意的是**，配置参数也应该被证明和记录，以避免"巫毒常数"（Ousterhout 定律）。**关键问题在于**，如果您不知道正确的值，Claude 如何确定它？
...
**基于预制组件理论**，即使 Claude 可以编写脚本，预制脚本也提供优势：
...
**这个图清楚地显示了**，可执行脚本如何与说明文件一起工作。说明文件（forms.md）引用脚本，Claude 可以执行它而无需将其内容加载到上下文中。
...
**实践证明**，对于大多数实用脚本，执行是首选，**因为**它更可靠和高效。有关脚本执行如何工作的详情，请参阅下面的[运行时环境](#runtime-environment)部分。
...
**基于多模态理论**，当输入可以呈现为图像时，让 Claude 分析它们：
...
**基于风险控制理论**，当 Claude 执行复杂的开放式任务时，它可能会犯错误。"计划-验证-执行"模式通过让 Claude 首先以结构化格式创建计划，然后在执行前使用脚本验证该计划来及早捕获错误。

**示例**：想象要求 Claude 根据电子表格更新 PDF 中的 50 个表单字段。**没有验证的话**，Claude 可能会引用不存在的字段、创建冲突的值、遗漏必需字段或错误地应用更新。

**解决方案**：使用上面显示的工作流模式（PDF 表单填充），但添加一个中间 `changes.json` 文件，在应用更改前进行验证。**因此**，工作流变成：分析 → **创建计划文件** → **验证计划** → 执行 → 验证。
...
**基于平台适配理论**，Skill 在代码执行环境中运行，具有特定于平台的限制：

* **claude.ai**：可以从 npm 和 PyPI 安装包并从 GitHub 存储库拉取
* **Anthropic API**：没有网络访问权限，没有运行时包安装

**因此**，在您的 SKILL.md 中列出所需的包，并验证它们在[代码执行工具文档](/zh-CN/docs/agents-and-tools/tool-use/code-execution-tool)中可用。
...
3. **高效执行脚本**：实用脚本可以通过 bash 执行，而无需将其完整内容加载到上下文中。**重要的是**，只有脚本的输出消耗令牌
...
**这个例子清楚地说明**，当用户询问收入时，Claude 读取 SKILL.md，看到对 `reference/finance.md` 的参考，并调用 bash 来仅读取该文件。sales.md 和 product.md 文件保留在文件系统上，在需要前消耗零上下文令牌。**因此**，这个基于文件系统的模型是启用渐进式披露的原因。Claude 可以导航并有选择地加载每个任务所需的内容。
...
**基于服务发现理论**，如果您的 Skill 使用 MCP（模型上下文协议）工具，始终使用完全限定的工具名称以避免"找不到工具"错误。
...
**需要注意的是**，没有服务器前缀，Claude 可能无法定位工具，特别是当有多个 MCP 服务器可用时。
...
**基于性能优化理论**，保持 SKILL.md 正文在 500 行以下以获得最佳性能。**如果**您的内容超过此限制，使用前面描述的渐进式披露模式将其拆分为单独的文件。有关架构详情，请参阅[Skill 概述](/zh-CN/docs/agents-and-tools/agent-skills/overview#how-skills-work)。
