...
This command reads text content from the specified input file, analyzes its semantic structure, and generates hierarchical chapter titles with proper markdown formatting. The resulting markdown content is printed directly to standard output.
...
### 输出格式要求
- **标准输出**：将处理后的完整 markdown 内容直接输出到标准输出
